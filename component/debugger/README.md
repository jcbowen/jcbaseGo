...
- ✅ **增强型 JSON 查看器**: 详情页集成 VSCode 同款 `jsonc-parser`，支持 JSON 美化、层级折叠/展开、自动修复（注释/尾部逗号等）、原始/修复后切换、一键复制
...
详情页对 JSON 内容（请求体、响应体、流式响应数据、Logger 日志、会话数据）提供了类 VSCode 的查看体验：

- **JSON 美化**：自动缩进格式化并语法高亮
- **层级折叠/展开**：对象 `{}` 与数组 `[]` 支持点击箭头折叠，提供「全部展开」「全部折叠」按钮
- **自动修复**：基于 VSCode 同款 `jsonc-parser` 库，自动处理注释、尾部逗号等常见非标准 JSON
- **原始/修复后切换**：当 JSON 存在格式问题时，可在原始内容与修复后内容之间切换查看
- **一键复制**：支持复制原始或修复后的 JSON 内容

JSON 查看器依赖的 `jsonc-parser` 静态资源已内嵌到组件中（`component/debugger/static/jsonc-parser.bundle.js`），无需依赖外部 CDN。
