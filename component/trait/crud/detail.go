...
func (t *Trait) ActionDetail(c *gin.Context) {
	ctx := t.InitCrud(c, "detail")

	// detail参数获取回调
	callResults := t.callCustomMethod("DetailFormData", ctx)
	...
	// 为了方便，直接传query进去拼接就好
	query = t.callCustomMethod("DetailSelect", ctx, query)[0].(*gorm.DB)
	...
	query = t.callCustomMethod("DetailQuery", ctx, query, mapData)[0].(*gorm.DB)
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			ctx.Result(errcode.NotExist, "数据不存在或已被删除")
		} else {
	...
	// 调用自定义的Detail方法处理结果
	result = t.callCustomMethod("Detail", ctx, result)[0]

	// 返回结果
	t.callCustomMethod("DetailReturn", ctx, result)
}
...
//   - error: 处理过程中的错误信息
func (t *Trait) DetailFormData(ctx *Context) (map[string]any, error) {
	// 获取安全过滤后的请求参数
...
func (t *Trait) DetailSelect(ctx *Context, query *gorm.DB) *gorm.DB {
...
func (t *Trait) DetailQuery(ctx *Context, query *gorm.DB, mapData map[string]any) *gorm.DB {
	return query
}
...
// 返回值：
//   - interface{}: 处理后的详情数据
func (t *Trait) Detail(ctx *Context, item interface{}) interface{} {
	return item
}
...
func (t *Trait) DetailReturn(ctx *Context, detail interface{}) bool {
	ctx.Result(errcode.Success, "ok", detail)
	return true
}
